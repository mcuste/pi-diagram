...
The renderer is not built yet. It arrives as these modules (source normalization, safe-subset
preflight, D2 process runner, profiles, diagnostics, render selection, cache, artifact writing),
with a real-D2 integration suite in `test/*.e2e.mjs` and a `pnpm test:integration` script to run
it separately from the deterministic suite. The D2 CLI version this project supports is pinned
at that point; nothing here spawns `d2` today.
