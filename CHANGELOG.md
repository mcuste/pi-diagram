...
- Labels with a line break, such as `\n` in a quoted label, now render as SVG and PNG instead
  of falling back to Unicode-only output.

## [0.6.2] - 2026-09-07

### Fixed

- Resuming a session that contains a diagram no longer stops Pi 0.85 and later. Every display
  component now clears its cached lines when the host asks it to.
...
- Package metadata now points the Pi and Oh My Pi galleries at a demo video and a preview
  image.
...
- Reframed the project around agents leading with token-efficient D2 diagrams when visual
  explanations are clearer than prose.
